...
def main():
    ...
    ap.add_argument("--tol", type=float, default=1e-11, help="relative L2 residual tolerance (||b-Ax||_2 < tol*||b||_2), matches the C++ benchmark's kIterativeTolerance -- verified this is the same convergence formula Eigen's setTolerance() uses, so a given value means the same thing on both sides. 1e-11 (not 1e-7) because a 1e-7 relative-L2 target can still leave a much larger absolute L-infinity residual on a large vector when the residual isn't evenly distributed across components -- measured 1.5e-4 at 1e-7 on the real dragon mesh's harmonic system, still 1.5-2.8e-7 at 1e-10; 1e-11 reliably gets L-infinity under 1e-7 with solid margin at negligible extra iteration cost")
